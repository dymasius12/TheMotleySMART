...
def analyze_stock_parameters(stock_info):
    """
    Analyze stock based on the criteria provided:
    - Profitability
    - Stability
    - Credibility
    """
    ...
    if roe >= 0.12:
        analysis_results['Profitability'] = 'Yes'
    
    # Stability Analysis
    beta = stock_info.get('beta', 0)
    if 0.8 < beta < 1.3:
        analysis_results['Stability'] = 'Yes'
    ...
    if debt_ratio > 0.8:
        analysis_results['Credibility'] = 'Yes'
    
    return analysis_results

def display_stock_analysis():
    """Display the stock analysis results in a new window."""
    input_value = stock_code_entry.get()
    if not input_value:
        messagebox.showerror("Error", "Please enter a stock code or company name.")
        return
    ...
    for i, (parameter, result) in enumerate(analysis_results.items()):
        tk.Label(analysis_window, text=f"{parameter}: {result}", font=('Arial', 12)).grid(row=i, column=0, padx=10, pady=10)
